...
class Ibu(models.Model):
    ...
    nik = models.CharField(
        verbose_name="NIK", max_length=16, unique=True, help_text=nik_help_text
    )
    nama = models.CharField(max_length=255)
    alamat = models.TextField(null=True, blank=True)
    nomor_telepon = models.CharField(
        max_length=15, null=True, blank=True, help_text="format 08XXXXXX..."
    )
...
class Bidan(models.Model):
    ...
    str = models.CharField(
        verbose_name="STR", max_length=255, default="0", help_text=str_help_text
    )
    nomor_telepon = models.CharField(
        max_length=15, null=True, blank=True, help_text="format 08XXXXXX..."
    )
...
class Bayi(models.Model):
    ...
    bidans = models.ManyToManyField(to="Bidan", related_name="bayis", blank=True)
...
class Imunisasi(models.Model):
    ...
    maksimum_usia = models.IntegerField(
        blank=True, null=True, help_text=help_text_maksimum_usia
    )
...
class ImunisasiDiberikan(models.Model):
    ...
    jam_pencatatan = models.TimeField(
        null=True, blank=True, verbose_name="jam pencatatan (WIB)"
    )
    # bidan yg menginput
    bidan = models.ForeignKey(
        ...
        null=True,
        related_name="imunisasi_dilakukan",
    )
